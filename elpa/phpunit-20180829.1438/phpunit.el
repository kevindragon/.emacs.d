...
(defun phpunit--configured-executable ()
  (let* ((dir (phpunit-get-root-directory))
         (conf-file (s-concat dir "/.phpunit")))
...
(defun phpunit--find-executable (directory)
  ...
  (let ((executable (or (phpunit--configured-executable)
                        phpunit-executable
                        phpunit-default-program)))
